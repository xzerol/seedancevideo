...
export async function GET(
  ...
  if (!batch) {
    return NextResponse.json({ error: "生成批次不存在" }, { status: 404 });
  }
  ...
  const updatedTasks = await Promise.all(
    batch.tasks.map(async (task) => {
        ...
        if (task.status === "completed" && task.videoUrl && !isLocalVideoUrl(task.videoUrl)) {
          try {
            const videoUrl = await ensureLocalVideoFile(
              task.videoUrl,
              `seedance-${task.providerTaskId}.mp4`
            );
      ...
      try {
        const providerTask = await getSeedanceTask(task.providerTaskId);
        return prisma.generationTask.update({
          where: { id: task.id },
          data: {
        ...
        return prisma.generationTask.update({
          ...
          data: {
            status: "failed",
            errorMessage:
              error instanceof Error ? error.message : "查询 Seedance 任务失败"
          }
        });
      }
    })
